...
# get data for {mpcDesignation}
r = requests.post(
    '{APIURL}/api/v1/sso',
    json={{
        'n_or_d': '{mpcDesignation}',
        'withEphem': True,
        'withResiduals': True,
        'output-format': 'json'
    }}
...
curl -H "Content-Type: application/json" -X POST \\
    -d '{{"n_or_d":"{mpcDesignation}", "output-format":"csv"}}' \\
    {APIURL}/api/v1/sso -o {mpcDesignation}.csv
    """
    ...
    if mpcDesignation != "null":
        data, kind = get_sso_data(mpcDesignation)
        if kind is not None:
            # from MPC
            card_properties = card_sso_mpc_params(data, mpcDesignation, kind)
        else:
